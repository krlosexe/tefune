...
public function registroUserAdmin()
	...
	function uploadprofile() {
		...
		$id_user      = $this->users_model->getUser($id);
		$name_file               =  $id_user->loginUsers;
        $mi_archivo              = 'files';
        ...
        if (!$this->upload->do_upload($mi_archivo)) {
            ...
            $this->session->set_flashdata('error', $this->upload->display_errors());
           	redirect(base_url()."cuenta/users/profile/");
            return;
        }else{
        	$data = $this->upload->data();
        	$datos = array('fotos' => $data["file_name"]);
        	if ($this->users_model->update($id, $datos)) {
				$this->session->set_flashdata('valid', "Su imagen se cargo Exitosamente");
	           	redirect(base_url()."cuenta/users/profile/");
        	}
        }
    }
